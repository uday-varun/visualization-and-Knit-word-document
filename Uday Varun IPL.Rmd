...
# 7. Data Manipulation:
Along with loading the CSV file I am also assigning the empty cells with ‘NA’. The dataset contains 756 rows which tells us that in total 756 matches being played from 2008 to 2009. out of these 756 matches 4 matches do not have any results as these matches were dismissed due to rain. Hence, these 4 empty columns were replaced by ‘NA’ while importing the dataset so that it would not affect the visualization as the ‘NA’ can be filtered out.  
```{r}
